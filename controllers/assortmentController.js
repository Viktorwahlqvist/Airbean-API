...
export const getAllProducts = (req, res) => {
  try {
    const stmt = db.prepare(
      "SELECT title, desc, price FROM items ORDER BY title ASC"
    ); // Bokstavsordning (kategori)
...
export const replaceProduct = (req, res) => {
  ...
  if (!title || !desc || !price) {
    return res
      .status(400)
      .json({ error: "Alla fält (title, desc, price) måste vara med" });
  }
...
export const getSortedItems = (req, res) => {
  ...
  const sortOrder =
    req.query.order && ["asc", "desc"].includes(req.query.order.toLowerCase()) // Converts provided value to lowercase
